function params() {
    global.help = function(com) {
        ...
        if (!com) {
            ...
            help.push(
                "  * hide                            - true or false. If true, than you will not see resources if you don't have they. NOT NECESSARY. Default: false"
            );
        }
    ...
    global.pushNotification = function(room) {
        ...
        console.log(
            `<a target="_blank" href="https://screeps.com/a/#!/room/${Game.shard.name}/${room}">${room} is in trouble. Click on me to open this room!</a>`
        );
        return notification;
    };
    ...
    global.progressBar = function(num = 0) {
        ...
        let progressBar =
            ...
            '" height="' +
            (height + 2) +
            '"> <rect x="1" y="1" fill="#F93842" width="' +
            (width / 100) * num +
            '" height="' +
            height +
            '"/> <rect x="1" y="1" stroke="#777777" stroke-width="2" fill="transparent" width="' +
            width +
            '" height="' +
            height +
            '"/> </svg>';
    ...
    global.svgCreep = function(body, width = 48, height = 48) {
        ...
        else if (claimCount > 0 && rangedAttackCount < 1 && attackCount < 1 && workCount < 1 && healCount > 0)
        ...
        if (moveCount > 0)
            bodySvg.push(
                '<circle cx="0" cy="0" r="4" transform="rotate(' +
                    rotateMove +
                    ')" stroke-width="8" stroke-dasharray="' +
                    percentMove +
                    ', 26" stroke="#A9B7C6" fill="none" />'
            );
        if (claimCount > 0)
            bodySvg.push(
                '<circle cx="0" cy="0" r="4" transform="rotate(' +
        ...
        if (workCount > 0)
            bodySvg.push(
                '<circle cx="0" cy="0" r="4" transform="rotate(' +
                    ...
                    ')" stroke-width="8" stroke-dasharray="' +
                    percentWork +
                    ', 26" stroke="#FFE56D" fill="none" />'
            );
        //body.push('<svg viewBox="0 0 ' + width + ' ' + height + '"> <circle cx="' + cx + '" cy="' + cy + '" r="' + r + '" stroke="black" stroke-width="1" fill="#202020"/>')
        bodySvg.push(
            '<image x="-5.7" y="-5.7" width="11.5" height="11.5" xlink:href="' +
                playerLink() +
                '"/> <circle cx="0" cy="0" r="5.8" stroke-width="0.6" stroke="black" fill="none"/>'
        );
        body.push("</svg>");
            ...
            if (spawn.room.storage) {
                ...
                info.push(
                    "<caption>STORAGE\n" +
                        progressBar(
                            Math.round(
                                (spawn.room.storage.store.getUsedCapacity() /
                                    spawn.room.storage.store.getFreeCapacity()) *
                                    100
                            )
                        ) +
                        "\n(" +
                        Math.round(
                            (spawn.room.storage.store.getUsedCapacity() / spawn.room.storage.store.getFreeCapacity()) *
                                100
                        ) +
                        "%)\n\n</caption>"
                );
                ...
                for (z in resources) {
                    if (spawn.room.storage.store[resources[z]] > 0) {
                        info.push("<tr>");
                        info.push("<td> " + resourceImg(resources[z]) + "</td>");
                        info.push("<td> " + spawn.room.storage.store[resources[z]] / 1000 + "</td>");
                        i;
            ...
            if (spawn.room.terminal) {
                ...
                for (z in resources) {
                    if (spawn.room.terminal.store[resources[z]] > 0) {
                        info.push("<tr>");
                        info.push("<td> " + resourceImg(resources[z]) + "</td>");
                        info.push("<td> " + spawn.room.terminal.store[resources[z]] / 1000 + "</td>");
                        i;
