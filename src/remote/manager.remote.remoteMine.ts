...
export class RemoteMineManager {
    ...
    private removeUnusedHaulers() {
        for (const hauler of this.mine.haulers!) {
            if (!Game.creeps[hauler]) {
                _.remove(this.mine.haulers!, (h) => { return h === hauler; });
            }
        }
    }
    ...
    private reassignContainer(): boolean {
        const containerRoomName = _.last(_.keys(this.mine.pathingLookup));
        if (containerRoomName) {
            const endPos = _.last(this.mine.pathingLookup[containerRoomName][0])
            if (endPos) {
                const containerPos = new RoomPosition(endPos.x, endPos.y, containerRoomName);
                if (containerPos) {
                    const container: Structure | undefined = containerPos.lookFor(LOOK_STRUCTURES).find(object => object.structureType === STRUCTURE_CONTAINER);
                    if (container) {
    ...
    private rebuildContainer() {
        ...
        if (containerRoomName) {
            ...
            if (endPos) {
                ...
                this.maintainActiveRemoteBuilder(containerRoomName);
            }
        }
    }
    ...
    private maintainActiveRemoteBuilder(containerRoomName: string) {
        if (!this.getActiveRemoteBuilder(containerRoomName)) {
            ...
            dcr.createdDedicatedCreepRequest(containerRoomName, CreepRole.builder, `${this.spawn.name}_BPR_${containerRoomName}`, undefined, true);
        }
    }
    ...
    private requestMiner() {
        ...
        const remoteRoomName: string | undefined = _.last(_.keys(this.mine.pathingLookup));
        if (remoteRoomName) {
            const minerName: string = `dMiner${this.room.name}${Game.time}`;
            const dcr: DedicatedCreepRequester = new DedicatedCreepRequester(this.spawn);
            dcr.createdDedicatedCreepRequest(this.mine.containerId!, CreepRole.dropper, minerName, this.mine.vein, true, { target: remoteRoomName, independentOperator: false });
            this.mine.miner = minerName;
        }
        else {
            console.log(`Could not determine room target for mine: ${JSON.stringify(this.mine)}`)
        }
    }
    ...
    private requestHauler() {
        ...
        if (remoteRoomName) {
            ...
            const dcr: DedicatedCreepRequester = new DedicatedCreepRequester(this.spawn);
            dcr.createdDedicatedCreepRequest(this.mine.containerId!, CreepRole.hauler, haulerName, undefined, true, { target: remoteRoomName, independentOperator: false });
