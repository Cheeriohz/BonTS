...
export class ExpeditionManager {
        ...
        if (assignedExpedition) {
            if (Game.spawns[assignedExpedition.spawnOrigin].room.name !== creep.room.name) {
                if (findings && findings.length > 0) {
                    const boxedFindings: ExpeditionFoundTarget[] = _.map(findings, f => {
                        return { id: f, roomName: creep.room.name };
                    });
                    assignedExpedition.progress.foundTargets = _.concat(
                        _.difference(boxedFindings, assignedExpedition.progress.foundTargets),
                        assignedExpedition.progress.foundTargets
                    );
                }
            }
        ...
        if (creep.memory.orders) {
            ...
            console.log(
                ErrorMapper.sourceMappedStackTrace(
                    "You need to ensure that we actually have a target before you create an order"
                )
            );
        }
    }
        ...
        if (creep.memory?.orders) {
            ...
            if (this.determineTargetLeaf(expedition.progress.searchTreeOriginNode, creep)) {
                ...
                if (expedition.progress.foundTargets.length > 0) {
                    console.log(
                        `Ending expedition, found targets: ${JSON.stringify(expedition.progress.foundTargets)}`
                    );
    ...
    public endExpedition(expedition: Expedition) {
        if (expedition.progress.foundTargets.length === 0) {
            ...
            this.createAndStoreExpeditionResults(
                expedition.expeditionTypeName,
                expedition.progress.foundTargets,
                Game.spawns[expedition.spawnOrigin]
            );
            _.remove(Memory.expeditions, e => e === expedition);
        }
    }
    ...
    private createAndStoreExpeditionResults(
        ...
        if (typeof ExpeditionResultsHandler !== "undefined") {
            const expeditionResultsHandler = new ExpeditionResultsHandler(targets, expeditionTypeName);
    ...
    private expandExpedition(expedition: Expedition) {
        ...
        if (expedition.progress.searchDepth === expedition.progress.maxDepth) {
            // ("Max search depth has been reached for expedition.");
            this.endExpedition(expedition);
        } else {
    ...
    private recursiveExpand(tree: ScreepsSearchTree, expedition: Expedition): ScreepsSearchTree {
        if (tree.children.length === 0) {
            ...
            let roomsToAdd = _.compact(
                _.difference(_.values(Game.map.describeExits(tree.nodeName)), expedition.progress.plottedRooms)
            );
            if (roomsToAdd) {
                expedition.progress.plottedRooms = _.concat(
                    expedition.progress.plottedRooms,
                    this.addLeaves(roomsToAdd, expedition, tree)
                );
            }
            return tree;
        } else {
    ...
    private orphanCreep(creep: Creep) {
        creep.say("💤 💤 💤");
        creep.memory.working = false;
    }
}
